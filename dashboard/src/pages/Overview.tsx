...
const SEMANTIC_VARS = ['--primary', '--destructive', '--muted-foreground', '--muted', '--border', '--card', '--card-foreground'] as const
...
function useCssColors(): { chart: string[]; semantic: SemanticColors } {
  const resolve = useCallback(() => {
    ...
    const chart = Array.from({ length: CHART_VAR_COUNT }, (_, i) =>
...
export function Overview({ keys, logs, trafficTick, fullStats }: { keys: KeyEntry[]; logs: LogEntry[]; trafficTick: number; fullStats: any }) {
  ...
  const avgDaily = dailyEntries.length > 0
    ? Math.round(dailyEntries.reduce((sum, [, d]: [string, any]) => sum + (d.requests ?? 0), 0) / dailyEntries.length)
    : 0
  ...
  const modelEntries = fullStats ? (Object.entries(fullStats.models ?? {})
  ...
  return (
    <div className="p-8 w-full space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard label="Success Rate" value={`${successRate}% `} sub={`${totalSuccess} successful`} accent={successRate >= 90 ? 'text-green-500' : successRate >= 70 ? 'text-yellow-500' : 'text-destructive'} />
        <StatCard label="Errors" value={totalErrors} sub="Total errors" accent={totalErrors > 0 ? 'text-destructive' : undefined} />
        <StatCard label="Avg Daily" value={avgDaily} sub={`over ${dailyEntries.length} day${dailyEntries.length !== 1 ? 's' : ''} `} />
      </div>
        ...
        <div style={{ height: 220 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trafficHistory} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              />
              <Tooltip
                contentStyle={{ borderRadius: '12px', border: `1px solid ${sc['--border']}`, backgroundColor: sc['--card'], color: sc['--card-foreground'], boxShadow: 'var(--shadow-md)' }}
                itemStyle={{ color: sc['--primary'], fontWeight: 600 }}
                cursor={{ stroke: sc['--muted-foreground'], strokeWidth: 1, strokeDasharray: '4 4', opacity: 0.4 }}
              />
              <Area
                type="monotone"
                dataKey="reqs"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-border/50 bg-card p-6 shadow-sm">
          ...
          {days.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">No request history yet.</p>
          ) : (
            <div style={{ height: 250 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={days} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="success" name="Success" fill={sc['--primary']} radius={[4, 4, 0, 0] as any} />
                  <Bar dataKey="errors" name="Errors" fill={sc['--destructive']} radius={[4, 4, 0, 0] as any} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex-1 overflow-auto overflow-x-auto min-h-0 border border-border/50 rounded-lg">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {keys.map(k => (
                  <tr key={k.tail} className="border-b border-border last:border-0 hover:bg-muted/30">
                    <td className="px-3 py-2 font-mono">...{k.tail}</td>
                    <td className="px-3 py-2">
                      <span className={`inline - flex items - center px - 1.5 py - 0.5 rounded - full text - [10px] font - medium ${statusColors[k.status] ?? 'text-muted-foreground bg-muted'} `}>
                        {k.status}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums">{k.usage ?? 0}</td>
                    <td className="px-3 py-2 text-right tabular-nums text-destructive">{k.errors ?? 0}</td>
        ...
        <div className="rounded-2xl border border-border/50 bg-card p-6 flex flex-col h-[450px] shadow-sm">
          ...
          <div
            ...
            style={{ backgroundColor: 'oklch(0.22 0.008 50)', border: '1px solid oklch(0.30 0.008 50)' }}
          >
            {displayLogs.length === 0 ? (
              <div style={{ color: 'oklch(0.45 0.008 50)' }} className="italic">No logs yet...</div>
            ) : (
              displayLogs.map((log, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{ backgroundColor: idx % 2 === 0 ? 'oklch(0.25 0.008 50 / 0.6)' : 'transparent' }}
                  >
                    <span className="shrink-0 tabular-nums" style={{ color: 'oklch(0.45 0.008 50)' }}>
                      {log.timestamp}
                    </span>
